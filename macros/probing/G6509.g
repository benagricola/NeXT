; G6509.g: INSIDE CORNER PROBE (NEW)
;
; Probes two perpendicular inside surfaces to find the corner intersection point.
; This is the opposite of G6508 - probes surfaces in positive directions.
; Logs the corner coordinates to the Probe Results Table.
;
; USAGE: G6509 [X<surface1>] [Y<surface2>] [F<feedrate>] [R<retries>]
;
; Parameters:
;   X: X-axis surface coordinate to probe (positive direction from current position)
;   Y: Y-axis surface coordinate to probe (positive direction from current position)
;   F: Optional feedrate override
;   R: Optional number of probe retries
;
; Note: This implementation assumes probing in positive X and Y directions from current position
...
if { !global.nxtFeatureTouchProbe }
    abort "G6509: Touch probe feature is disabled"

if { state.currentTool != global.nxtProbeToolID }
    abort "G6509: Touch probe must be selected for inside corner probing"

; Default parameters - probe 10mm in positive direction if not specified
...
echo { "G6509: Starting position: X=" ^ var.startX ^ " Y=" ^ var.startY }

; First probe: X-axis surface (probing in positive X direction)
...
; Second probe: Y-axis surface (probing in positive Y direction)
...
echo { "G6509: Inside corner probe complete. Result logged to slot " ^ var.resultSlot }
echo { "G6509: Corner found at X=" ^ var.cornerX ^ " Y=" ^ var.cornerY ^ " Z=" ^ var.cornerZ }